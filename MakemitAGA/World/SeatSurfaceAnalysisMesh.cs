...
namespace MakemitAGA.World
{
    ...
    internal static partial class SeatSurfaceAnalysisRuntime
    {
        ...
        // v0.3.4：床垫侧边并不是理想的几何直角，床单、软垫圆角以及高度图平滑
        // 会让靠近边缘的一小圈表面出现轻微坡度和高度起伏。旧参数对这部分略显保守，
        // 导致视觉上仍然足够平缓的外侧区域被标为红色，紫色动作带整体被推向床中心。
        //
        // 这里采用“轻微放宽，而不是取消检查”的策略：
        //   - 支撑覆盖率仍保持 0.80，不允许臀部大面积悬空；
        //   - 允许测试区域内的高度变化由 0.105m 增到 0.125m；
        //   - 法线 Y 下限由 0.62 降到 0.58，只多接受一小部分软垫圆角坡面。
        //
        // 后续仍有边缘距离、地板、NavMesh、身体胶囊、走廊和腿部空间检查，
        // 因此这不是把红区无条件变紫，而是让“看起来不算陡”的床沿先进入候选阶段。
        private const float SeatMinSupportCoverage = 0.80f;
        private const float SeatMaxPatchHeightRange = 0.125f;
        private const float SeatMinNormalY = 0.58f;
        ...
        // “边缘坐姿动作有效区”参数。
        //
        // 一个绿色承重点要进一步成为紫色动作点，至少需要满足：
        //   1. 它距离当前座面平台的边缘不能太近，也不能太远；
        //   2. 边缘外侧能找到真实地板与 NavMesh；
        //   3. 米塔的站立胶囊、接近走廊和腿部空间都不能被家具阻挡。
        //
        // 重要历史背景：
        //   旧扫描只保留床垫顶部，因此“surfaceMask 消失”基本就等于“床垫到边”。
        //   新扫描为了保留沙发靠背、扶手和坐垫，会把家具的多个高度层都留下。
        //   对床来说，床垫外侧下面仍可能存在床架；如果只看 surfaceMask，系统会误以为
        //   床垫还没有到边，最终一直找到整个床架的最外轮廓，导致床的紫色区域归零。
        //
        // ActionBoundaryMinDownwardDrop 用来补足这个语义：
        //   即使下一格仍有表面，只要它相对候选座面明显降低，也把这条高度断层视为
        //   “当前座面平台的边缘”。这里只识别向下落差，不把向上升高（例如沙发靠背）
        //   错当成外侧边缘。0.16m 高于常见扫描噪声和床面细小起伏，同时低于常见的
        //   床垫到床架落差；如果以后更换扫描分辨率或平滑强度，应优先重新验证此值。
        // ---------------------------------------------------------------------------------
        // ---------------------------------------------------------------------------------
        // 紫色动作带的“向外/向内宽度”。
        //
        // 这里描述的是臀部候选点距离当前座面平台边缘的水平距离：
        //
        //   ActionMinEdgeInset
        //     候选点至少要落在边缘内侧这么远。
        //     太靠外时，臀部容易悬在座面外，扫描噪声也更容易把点放到床沿之外。
        //
        //   ActionMaxEdgeInset
        //     候选点最多可以深入座面这么远。
        //     旧值 0.34m 会把紫色区域压成偏窄的“精确轨道”；这虽然安全，
        //     但不符合实际坐姿中“可以稍微靠前或靠后”的自然容差，也让 VLM
        //     在缩小后的辅助图上不容易点中紫色区域。
        //
        // v0.3.4 的目标不是继续把紫色带做得更宽，而是把整条带“向床沿外侧平移”：
        //   - 最小距离由 0.04m 降到 0.02m，让臀部候选点可以更靠近床沿；
        //   - 最大距离由 0.46m 收回到 0.40m，避免候选点深入床中央过多，
        //     也避免米塔的大腿需要跨越过长的座面深度。
        //
        // 这两个值控制的是候选点相对边缘的水平深度，并不改变最终站立点。
        // 真正的动作安全仍由后续检查保证：外侧地板、NavMesh、站立胶囊、
        // 接近走廊与腿部空间任何一项失败，该格仍然不会成为紫色。
        //
        // 调参建议：
        //   - 紫色整体仍偏里：先把 ActionMinEdgeInset 降到 0.01m，勿直接改成负数；
        //   - 臀部开始出现悬空：把 ActionMinEdgeInset 恢复到 0.03~0.04m；
        //   - 紫色仍深入床中央：把 ActionMaxEdgeInset 每次降低约 0.03~0.04m；
        //   - 不要优先放宽身体/NavMesh/腿部检查，那些才是真正的安全底线。
        // ---------------------------------------------------------------------------------
        private const float ActionMinEdgeInset = 0.02f;
        private const float ActionMaxEdgeInset = 0.40f;

        // 高度感知边缘阈值保持 v0.3.2 的 0.16m，不在本次修改中继续放宽。
        // 这样可以单独观察“动作带宽度”的变化，避免同时改变床垫边缘识别结果。
        private const float ActionBoundaryMinDownwardDrop = 0.16f;
